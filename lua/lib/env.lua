  ...
  snacks.picker.pick {
    ...
    actions = {
      copy_to_clipboard = function(picker, item)
        ...
        picker:close()
        vim.notify("Env - var < " .. item.text .. " > value copied to clipboard", vim.log.levels.INFO, { title = "Env" })
      end,
      add_environment_var = function(picker, item)
        local prompt = "Env - enter new env var in 'VAR=VALUE' format: "
        picker:close()
      ...
      edit_environment_var = function(picker)
        ...
        local prompt = "Env - enter new value for < " .. c_word .. " > : "
        picker:close()
