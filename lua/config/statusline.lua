...
local function get_line_onTot()
   return win_is_smaller(preset_width.row_onTot) and string.format(" %s%%l%s+%%L ", colors.git, colors.location)
...
local function get_lsp_diagnostic()
   ...
   local status_ok = (errors + warns + infos + hints) == 0
   ...
   -- if vim.bo.filetype == "python" then
      if not win_is_smaller(preset_width.git_branch) then
         ...
         if venv then
            ...
            local py_icon = '󰌠'
            return string.format("%s (%s)", py_icon, venv)
         end
      end
   -- end
   return ""
